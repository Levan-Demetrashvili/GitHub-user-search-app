...
function UserProvider({ children }: { children: React.ReactNode }) {
  ...
  async function getUserData() {
    ...
    try {
      setIsLoading(true);
      setError("");
      const res: AxiosResponse = await axios.get(API_URL + username, { signal: AbortSignal.timeout(5000) });
      if (!res.data) throw new Error("Something went wrong fetching user data ...");
      setUser(res.data);
    } catch (e) {
